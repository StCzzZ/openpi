...
class LiberoRunner:
    ...
    def __init__(self, args):
        ...
        self.policy_rollout_window_name_2 = "policy rollout view 2"
        self.policy_rollout_wrist_window_name_2 = "policy rollout wrist view 2"
        # cv2.namedWindow(self.policy_rollout_window_name_2, cv2.WINDOW_NORMAL)
        # cv2.namedWindow(self.policy_rollout_wrist_window_name_2, cv2.WINDOW_NORMAL)
        ...
        self.states_buffer = np.zeros((self.args.max_buffer_size, self.args.num_envs, state_init.shape[1]), dtype=float)
        ...
        while True:
            try:
                ...
                with self.lock:
                    ...
                    self.states_buffer[self.env_running_timesteps, self.running_env_indices] = sim_state_arr[self.running_env_indices]
        ...
        while not self.listening_stop_evt.is_set():
            ...
            while b"\n" in buffer:
                ...
                if isinstance(parsed, str) and parsed.lower() == "stop":
                    ...
                    if self.HIL_active: 
                        with self.env_lock:
                            self.env.reset(id=self.HIL_id.tolist())
                            ...
                            obs_stop = self.env.set_init_state(stop_init_states, id=self.HIL_id.tolist())
    ...
    def _HIL_loop(self):
        ...
        while not self.HIL_stop_evt.is_set():
            try:
                ...
                with self.env_lock:
                    obs_id, reward, done, info = self.env.step(action_arr, id=self.HIL_id.tolist())

                # record the action and observations into buffers at current env_timesteps
                t = int(self.env_timesteps[self.HIL_id])
                # clamp to buffer sizes
                if t < self.action_buffer.shape[0]:
                    with self.lock:
                        self.action_buffer[t, self.HIL_id] = action_arr
                ...
                if t < self.states_buffer.shape[0]:
                    with self.lock:
                        self.states_buffer[t, self.HIL_id] = sim_state
                # images: resize same as main loop
                ...
                if t < self.img_buffer.shape[0]:
                    with self.lock:
                        self.img_buffer[t, self.HIL_id] = img
                if t < self.wrist_img_buffer.shape[0]:
                    with self.lock:
                        self.wrist_img_buffer[t, self.HIL_id] = wrist_img
                
                cprint(f"Recording at timestep {t} for env {self.HIL_id}", "magenta")

                # also try to record sim state from env.get_sim_state() if available
                with self.env_lock:
                    rec_states = np.array(self.env.get_sim_state())
                if t < self.state_record_buffer.shape[0]:
                    with self.lock:
                        self.state_record_buffer[t, self.HIL_id] = rec_states[self.HIL_id]
                
                cprint(f"rec_states.shape: {rec_states.shape}", "magenta")

                # increment timestep for that env (HIL stepping)
                with self.lock:
                ...
                if self.dones[self.HIL_id]:
                    ...
                    with self.env_lock:
                        ...
                        obs_success = self.env.set_init_state(success_init_states, id=self.HIL_id.tolist())
