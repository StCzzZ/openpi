...
@dataclasses.dataclass(frozen=True)
class MySequenceInputs(transforms.DataTransformFn):
    """
    This class is used to convert inputs to the model to the expected format. It is used for both training and inference.
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # The current observation and the next observation shall both be included in this case
        assert data["observation/image"].shape[-4] == 2
        assert data["observation/wrist_image"].shape[-4] == 2
        assert data["observation/state"].shape[-2] == 2

        base_image = _parse_image(data["observation/image"][..., 0, :, :, :])
        wrist_image = _parse_image(data["observation/wrist_image"][..., 0, :, :, :])
        next_base_image = _parse_image(data["observation/image"][..., 1, :, :, :])
        next_wrist_image = _parse_image(data["observation/wrist_image"][..., 1, :, :, :])
        ...
        inputs = {
            ...
            "next_state": data["observation/state"][..., 1, :],
            "image": {
                "base_0_rgb": base_image,
                "left_wrist_0_rgb": wrist_image,
                # Pad any non-existent images with zero-arrays of the appropriate shape.
...
@dataclasses.dataclass(frozen=True)
class MySequenceOutputs(transforms.DataTransformFn):
    """
    This class is used to convert outputs from the model back the the dataset specific format. It is
    used for inference only.
